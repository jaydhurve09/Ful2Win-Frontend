...
const GameTournaments = () => {
  ...
  const fetchGameAndTournaments = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        navigate('/login', { state: { from: `/game-tournaments/${gameId}` } });
        return;
      }
            ...
            getFilteredTournaments().map(tournament => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-semibold line-clamp-2">{tournament.name}</h3>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                  <div>
                    ...
                    <p className="text-yellow-500 font-medium">
                      {formatPrize(tournament.entryFee || 0, tournament.mode === 'coin')}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-400">Prize Pool</p>
                    <p className="text-green-400 font-medium">
                      {formatPrize(tournament.prizePool || 0, tournament.mode === 'coin')}
                    </p>
                  </div>
                  <div>
                ...
                <div className="bg-gray-800/50 rounded-full h-2 mb-4">
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, ((tournament.participants?.length || 0) / (tournament.maxParticipants || 1)) * 100)}%` 
                    }}
                  />
                </div>
                ...
                >
                  ...
                  {tournament.status === 'upcoming' ? 'View Details' : 'Join Tournament'}
                </button>
              </div>
            ))
          )}
